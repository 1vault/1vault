/** Read next_trade_id / next_position_id from on-chain Vault account (deployed layout). */
export async function fetchVaultTradeCursor(
  ...
  o += 32 + 8; // strategist + vault_id
  skipStr();
  skipStr();
  o += 32 + 1 + 32 * 5 + 32 + 32; // base_mint, mints, share, vta
  o += 8 * 4; // shares, assets, position_value, hwm
  o += 2 + 1 + 2; // perf_fee, book_mode, mgmt_fee
  o += 1 + 2 + 1 + 1; // status, max_slip, open, pending
...
/** Resume open_position when the latest trade is Executed but no vault_position exists yet. */
